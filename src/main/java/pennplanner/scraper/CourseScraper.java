...
public class CourseScraper {
    ...
    public static List<RawCourse> scrapeDepartment(String url) {
        ...
        try {
            ...
            for (Element block : courseBlocks) {
                ...
                if (courseId.equals("UNKNOWN")) continue;
                ...
                boolean offeredFall          = detectOfferedInFall(block);
                boolean offeredSpring        = detectOfferedInSpring(block);
                ...
                // Normalize multi-CU courses to 1 CU so they remain schedulable
                if (creditUnits > 2.0) creditUnits = 1.0;

                courses.add(new RawCourse(
                        courseId, courseName, creditUnits,
                        prerequisites, corequisites,
                        mutuallyExclusive, alsoOfferedAs,
                        offeredFall, offeredSpring,
                        notOfferedEveryYear, description
                ));
            }
    ...
    private static List<String> extractLineCourses(String fullText, String label) {
        ...
        String[] labels = {
                "Also Offered As", "Mutually Exclusive", "Prerequisite",
                "Corequisite", "Equivalent Course", "Activity", "Course Unit",
                "Fall or Spring", "Fall", "Spring", "Summer", "Not Offered Every Year"
        };

        int start = fullText.indexOf(label + ":");
        if (start == -1) return courses;
    ...
    private static String extractOfferingLine(Element block) {
        ...
        for (Element element : elements) {
            ...
            if (text.equals("Fall or Spring")) return "Fall or Spring";
            if (text.equals("Fall")) return "Fall";
            if (text.equals("Spring")) return "Spring";
        }

        String html = block.html();
        if (html.contains(">Fall or Spring<")) return "Fall or Spring";
        if (html.contains(">Fall<")) return "Fall";
        if (html.contains(">Spring<")) return "Spring";
        return "";
    }
    ...
    private static String extractDescription(Element block, String courseId, String courseName) {
        ...
        String[] stopMarkers = {
                "Fall or Spring", "Not Offered Every Year",
                "Also Offered As:", "Mutually Exclusive:",
                "Prerequisite:", "Corequisite:",
                "Equivalent Course:", "Activity:", "Course Unit"
        };

        int stopIndex = description.length();
        for (String marker : stopMarkers) {
            int index = description.indexOf(marker);
            if (index != -1 && index < stopIndex) stopIndex = index;
        }
