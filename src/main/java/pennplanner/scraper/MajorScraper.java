...
public class MajorScraper {
    ...
    public static List<MajorRequirement> scrapeMajorRequirements(MajorInfo major) {
        ...
        try {
            ...
            if (isBioengineeringMajor(major.name)) {
                ...
                if (isElectricalEngineeringMajor(major.name)) {
                    requirements = removeUnneededElectricalEngineeringParsedRules(requirements);

                    requirements.add(buildElectricalEngineeringIntermediateAdvancedEseElectiveRule(major.name));
                    requirements.add(buildElectricalEngineeringAdvancedEseCoursesRule(major.name));
                    requirements.add(buildElectricalEngineeringAdvancedEseCourseOptionRule(major.name));
                    requirements.add(buildElectricalEngineeringMathElectiveRule(major.name));
                    requirements.add(buildElectricalEngineeringMathOrNaturalScienceElectiveRule(major.name));
                    requirements.add(buildElectricalEngineeringNaturalScienceLabRule(major.name));
                    requirements.add(buildElectricalEngineeringMathNaturalScienceEngineeringRule(major.name));
                    requirements.add(buildElectricalEngineeringProfessionalElectiveRule(major.name));
                    requirements.add(buildElectricalEngineeringSocialScienceHumanitiesRule(major.name));
                }

                if (isMaterialsScienceEngineeringMajor(major.name)) {
                    requirements = removeUnneededMaterialsScienceEngineeringParsedRules(requirements);

                    requirements.add(buildMaterialsScienceEngineeringMathElectiveRule(major.name));
                    requirements.add(buildMaterialsScienceEngineeringMseElectiveRule(major.name));
                    requirements.add(buildMaterialsScienceEngineeringTechnicalElectiveRule(major.name));
                    requirements.add(buildMaterialsScienceEngineeringSocialScienceHumanitiesRule(major.name));
                }

                if (isMechanicalEngineeringAppliedMechanicsMajor(major.name)) {
                    requirements = removeUnneededMechanicalEngineeringAppliedMechanicsParsedRules(requirements);

                    requirements.add(buildMechanicalEngineeringAppliedMechanicsConcentrationRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsMathElectiveRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsMathOrNaturalScienceElectiveRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsMeamUpperLevelRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsTechnicalElectiveRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsSocialScienceRule(major.name));
                    requirements.add(buildMechanicalEngineeringAppliedMechanicsHumanitiesRule(major.name));
                }
    ...
    private static boolean isElectricalEngineeringMajor(String majorName) {
        String lower = majorName.toLowerCase();

        return lower.contains("electrical engineering")
                && lower.contains("bse");
    }

    private static boolean isMaterialsScienceEngineeringMajor(String majorName) {
        String lower = majorName.toLowerCase();

        return lower.contains("materials science and engineering")
                && lower.contains("bse");
    }

    private static boolean isMechanicalEngineeringAppliedMechanicsMajor(String majorName) {
        String lower = majorName.toLowerCase();

        return lower.contains("mechanical engineering and applied mechanics")
                && lower.contains("bse");
    }
    ...
    private static MajorRequirement buildElectricalEngineeringIntermediateAdvancedEseElectiveRule(String majorName) {
        List<String> eseElectives = getEseCoursesAtOrAbove(2000);
        ...
        return new MajorRequirement(
                ...
                1,
                eseElectives,
                "Choose 1 Intermediate or Advanced ESE Elective. Includes ESE courses at the 2000-level or higher."
        );
    }

    private static MajorRequirement buildElectricalEngineeringAdvancedEseCoursesRule(String majorName) {
        List<List<String>> advancedEseGroups = groups(
                group("ESE 3190"),
                group("ESE 3400"),
                group("ESE 3500"),
                group("ESE 3600"),
                group("ESE 3700"),
                group("ESE 4190", "ESE 5720"),
                group("ESE 4730", "ESE 5730"),
                group("ESE 4750", "ESE 5750"),
                group("ESE 5150"),
                group("ESE 5160"),
                group("ESE 5180"),
                group("ESE 5190"),
                ...
                group("ESE 6500"),
                group("ESE 6740"),
                group("BE 5210", "CIS 4710", "CIS 5200")
        );
        ...
        return new MajorRequirement(
                ...
                4,
                advancedEseGroups,
                "Choose 4 advanced electives from the Advanced ESE course lists. Courses in the same option group are alternatives and should not both be selected.",
                true
        );
    }

    private static MajorRequirement buildElectricalEngineeringAdvancedEseCourseOptionRule(String majorName) {
        List<List<String>> advancedEseCourseOptionGroups = groups(
                group("ESE 2900", "ESE 2910"),
                group("ESE 3190"),
                group("ESE 3360"),
                group("ESE 3500"),
                group("ESE 4210"),
        ...
        return new MajorRequirement(
                majorName,
                "Advanced ESE Course Option",
                "CHOOSE_ONE_FROM_GROUPS",
                1,
                advancedEseCourseOptionGroups,
                "Choose one Advanced ESE course option. ESE 2900 and ESE 2910 must be taken together and count as one option.",
                true
        );
    }

    private static MajorRequirement buildElectricalEngineeringMathElectiveRule(String majorName) {
        List<String> mathCourses = scrapeMathCoursesFromCatalog();
        ...
        return new MajorRequirement(
                ...
                1,
                mathCourses,
                "Choose 1 Math Elective from the approved SEAS Mathematics courses list."
        );
    }

    private static MajorRequirement buildElectricalEngineeringMathOrNaturalScienceElectiveRule(String majorName) {
        List<String> options = getMathOrNaturalScienceCourses();
        ...
        return new MajorRequirement(
                ...
                1,
                options,
                "Choose 1 Math or Natural Science Elective. Combines approved Mathematics courses and Natural Science courses."
        );
    }

    private static MajorRequirement buildElectricalEngineeringNaturalScienceLabRule(String majorName) {
        List<String> options = new ArrayList<>(Arrays.asList(
                "BIOL 1124",
                "CHEM 1101",
                "MEAM 1470",
    ...
    private static MajorRequirement buildElectricalEngineeringMathNaturalScienceEngineeringRule(String majorName) {
        List<String> options = getMathNaturalScienceEngineeringCourses();

        return new MajorRequirement(
                majorName,
                "3 Math, Natural Science or Engineering courses",
                "CHOOSE_N",
                3,
                options,
                "Select 3 Math, Natural Science or Engineering courses. Combines approved Mathematics courses, Natural Science courses, and Engineering Elective courses."
        );
    }

    private static MajorRequirement buildElectricalEngineeringProfessionalElectiveRule(String majorName) {
        List<String> options = getElectricalEngineeringProfessionalElectiveCourses();
        ...
        return new MajorRequirement(
                ...
                1,
                options,
                "Choose 1 Professional Elective. Combines approved Mathematics, Natural Science, and Engineering courses with ESE 4000, EAS 5450, EAS 5950, MGMT 2370, and OIDD 2360."
        );
    }

    private static MajorRequirement buildElectricalEngineeringSocialScienceHumanitiesRule(String majorName) {
        List<String> sshCourses = scrapeSocialScienceHumanitiesCoursesFromCatalog();
    ...
    private static List<String> getElectricalEngineeringProfessionalElectiveCourses() {
        Set<String> courses = new LinkedHashSet<>();

        for (String course : getMathNaturalScienceEngineeringCourses()) {
            courses.add(course);
        }

        courses.add("ESE 4000");
        courses.add("EAS 5450");
        courses.add("EAS 5950");
        courses.add("MGMT 2370");
        courses.add("OIDD 2360");

        return new ArrayList<>(courses);
    }

    private static List<String> getEseCoursesAtOrAbove(int minimumNumber) {
        Set<String> courses = new LinkedHashSet<>();
    ...
    private static List<MajorRequirement> removeUnneededMechanicalEngineeringAppliedMechanicsParsedRules(
            List<MajorRequirement> requirements) {
        ...
        for (MajorRequirement req : requirements) {
            ...
            if (isMechanicalEngineeringAppliedMechanicsConcentrationSection(sectionName)) {
                continue;
            }

            if (note.contains("meam 3000-level breadth elective")
                    || note.contains("meam 5000-level")
                    || note.contains("breadth elective")) {
                continue;
            }

            if (sectionName.equals("concentration")
                    || note.contains("select only one track from the options below")
                    || note.contains("meam concentration form")
                    || note.contains("general concentration")) {
                continue;
            }
            ...
            if (note.contains("math elective")
                    || note.contains("mathematics elective")
                    || sectionName.contains("math elective")
                    || sectionName.contains("mathematics elective")) {
                continue;
            }

            if (note.contains("meam upper level")
                    || note.contains("meam upper-level")
                    || note.contains("upper level meam")
                    || note.contains("upper-level meam")
                    || (note.contains("5000") && note.contains("meam"))
                    || sectionName.contains("meam upper level")
                    || sectionName.contains("meam upper-level")
                    || sectionName.contains("upper level meam")
                    || sectionName.contains("upper-level meam")) {
                continue;
            }
            ...
            if (note.startsWith("select 1 social science course")
                    || note.startsWith("select one social science course")
                    || note.contains("select 1 social science course")
                    || note.contains("select one social science course")) {
                continue;
            }

            if (note.startsWith("select 2 humanities courses")
                    || note.startsWith("select two humanities courses")
                    || note.contains("select 2 humanities courses")
                    || note.contains("select two humanities courses")) {
                continue;
            }

            if (note.contains("select 1 social science or humanities course")
                    || note.contains("select one social science or humanities course")
                    || note.contains("select 1 social sciences or humanities course")
                    || note.contains("select one social sciences or humanities course")
                    || ((sectionName.contains("general electives")
                    || sectionName.contains("general elective"))
                    && note.contains("social science or humanities"))) {
                continue;
    ...
    private static boolean isMechanicalEngineeringAppliedMechanicsConcentrationSection(String sectionName) {
        if (sectionName == null) {
            return false;
        }
        ...
        return normalized.equals("dynamics, controls, and robotics concentration")
                || normalized.equals("energy, fluids and thermal systems concentration")
                || normalized.equals("energy, fluids, and thermal systems concentration")
                || normalized.equals("general concentration")
                || normalized.equals("mechanics of materials, structures and design concentration")
                || normalized.equals("mechanics of materials, structures, and design concentration");
    }

    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsConcentrationRule(String majorName) {
        List<String> meam3000PlusCourses = getMeamCoursesAtOrAbove(3000);
        List<String> meam5000PlusCourses = getMeamCoursesAtOrAbove(5000);

        List<String> dynamicsControlsRobotics = new ArrayList<>();
        dynamicsControlsRobotics.add("MEAM 3200");
        dynamicsControlsRobotics.add("MEAM 3210");
        addAllUnique(dynamicsControlsRobotics, meam3000PlusCourses);
        addAllUnique(dynamicsControlsRobotics, meam5000PlusCourses);

        List<String> energyFluidsThermal = new ArrayList<>();
        energyFluidsThermal.add("MEAM 3020");
        energyFluidsThermal.add("MEAM 3330");
        addAllUnique(energyFluidsThermal, meam3000PlusCourses);
        addAllUnique(energyFluidsThermal, meam5000PlusCourses);

        List<String> generalConcentration = new ArrayList<>(Arrays.asList(
                "MEAM 3020",
                "MEAM 3210",
                "MEAM 3330",
                "MEAM 3540"
        ...
        List<String> mechanicsMaterialsStructuresDesign = new ArrayList<>();
        mechanicsMaterialsStructuresDesign.add("MEAM 3210");
        mechanicsMaterialsStructuresDesign.add("MEAM 3540");
        addAllUnique(mechanicsMaterialsStructuresDesign, meam3000PlusCourses);
        addAllUnique(mechanicsMaterialsStructuresDesign, meam5000PlusCourses);

        List<List<String>> concentrationGroups = groups(
                dynamicsControlsRobotics,
                energyFluidsThermal,
                generalConcentration,
                mechanicsMaterialsStructuresDesign
        );

        return new MajorRequirement(
                majorName,
                "Concentration",
                "CHOOSE_ONE_CONCENTRATION",
                4,
                concentrationGroups,
                "Choose one MEAM concentration. Dynamics, Controls, and Robotics requires MEAM 3200, MEAM 3210, one MEAM 3000+ course, and one MEAM 5000+ course. Energy, Fluids and Thermal Systems requires MEAM 3020, MEAM 3330, one MEAM 3000+ course, and one MEAM 5000+ course. General Concentration requires MEAM 3020, MEAM 3210, MEAM 3330, and MEAM 3540. Mechanics of Materials, Structures and Design requires MEAM 3210, MEAM 3540, one MEAM 3000+ course, and one MEAM 5000+ course.",
                true
        );
    }

    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsMathElectiveRule(String majorName) {
        List<String> mathCourses = scrapeMathCoursesFromCatalog();
    ...
    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsMathOrNaturalScienceElectiveRule(String majorName) {
        List<String> options = getMathOrNaturalScienceCourses();
    ...
    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsMeamUpperLevelRule(String majorName) {
        List<String> meamUpperLevelCourses = getMeamCoursesAtOrAbove(5000);
        ...
        return new MajorRequirement(
                ...
                "CHOOSE_N",
                2,
                meamUpperLevelCourses,
                "Choose 2 MEAM Upper Level courses from all MEAM courses at the 5000-level or higher."
        );
    }

    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsTechnicalElectiveRule(String majorName) {
        List<String> options = getMathNaturalScienceEngineeringCourses();
        ...
        return new MajorRequirement(
                ...
                4,
                options,
                "Choose 4 Technical Electives. Combines approved Mathematics courses, Natural Science courses, and Engineering Elective courses."
        );
    }

    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsSocialScienceRule(String majorName) {
        ...
        return new MajorRequirement(
                ...
                "Select 1 Social Science course from the approved Social Sciences section of the SEAS Social Sciences and Humanities Breadth list."
        );
    }

    private static MajorRequirement buildMechanicalEngineeringAppliedMechanicsHumanitiesRule(String majorName) {
    ...
    private static List<String> getMeamCoursesAtOrAbove(int minimumNumber) {
        Set<String> courses = new LinkedHashSet<>();
    ...
    private static List<MajorRequirement> removeUnneededMaterialsScienceEngineeringParsedRules(
            List<MajorRequirement> requirements) {
        ...
        for (MajorRequirement req : requirements) {
            ...
            if (note.contains("mse elective")
                    || note.contains("mse electives")
                    || sectionName.contains("mse elective")
                    || sectionName.contains("mse electives")) {
                continue;
            }
    ...
    private static MajorRequirement buildMaterialsScienceEngineeringMathElectiveRule(String majorName) {
        List<String> mathCourses = scrapeMathCoursesFromCatalog();
    ...
    private static MajorRequirement buildMaterialsScienceEngineeringMseElectiveRule(String majorName) {
        List<String> mseElectives = getMseElectiveCourses();
        ...
        return new MajorRequirement(
                ...
                2,
                mseElectives,
                "Choose 2 MSE Electives from all MSE courses."
        );
    }

    private static MajorRequirement buildMaterialsScienceEngineeringTechnicalElectiveRule(String majorName) {
        List<String> options = getMathNaturalScienceEngineeringCourses();
        ...
        return new MajorRequirement(
                ...
                2,
                options,
                "Choose 2 Technical Electives. Combines approved Mathematics courses, Natural Science courses, and Engineering Elective courses."
        );
    }

    private static MajorRequirement buildMaterialsScienceEngineeringSocialScienceHumanitiesRule(String majorName) {
        List<String> sshCourses = scrapeSocialScienceHumanitiesCoursesFromCatalog();
    ...
    private static List<String> getMseElectiveCourses() {
        Set<String> electives = new LinkedHashSet<>();
    ...
    private static List<MajorRequirement> removeUnneededElectricalEngineeringParsedRules(
            List<MajorRequirement> requirements) {
        ...
        for (MajorRequirement req : requirements) {
            ...
            if (note.contains("intermediate or advanced ese elective")
                    || note.contains("intermediate/advanced ese elective")
                    || note.contains("advanced ese elective")
                    || sectionName.contains("intermediate or advanced ese elective")
                    || sectionName.contains("intermediate/advanced ese elective")
                    || sectionName.contains("advanced ese elective")) {
                continue;
            }

            if (isElectricalEngineeringAdvancedEseParsedRule(req)) {
                continue;
            }

            if (isElectricalEngineeringAdvancedEseCourseOptionParsedRule(req)) {
                continue;
            }
            ...
            if (note.contains("select 3 math, natural science or engineering courses")
                    || note.contains("select 3 math, natural science, or engineering courses")
                    || note.contains("select three math, natural science or engineering courses")
                    || note.contains("select three math, natural science, or engineering courses")
                    || note.contains("math, natural science or engineering")
                    || note.contains("math, natural science, or engineering")
                    || (note.contains("math") && note.contains("natural science") && note.contains("engineering"))
            ...
            if (sectionName.equals("professional electives 6")) {
                continue;
            }

            if (note.contains("professional elective - select from the following")) {
                continue;
            }

            if (isElectricalEngineeringProfessionalElectiveParsedCourseRow(req)) {
                continue;
            }

            if (note.contains("professional elective")
                    || note.contains("professional electives")
                    || sectionName.contains("professional elective")
                    || sectionName.contains("professional electives")) {
                continue;
            }
    ...
    private static boolean isElectricalEngineeringAdvancedEseParsedRule(MajorRequirement req) {
        if (req.sectionName == null) {
            return false;
        }
        ...
        if (note.contains("choose four advanced electives")) {
            return true;
        }
        ...
        Set<String> advancedEseCourses = new HashSet<>(Arrays.asList(
                "ESE 3190",
                "ESE 3400",
                "ESE 3500",
                "ESE 3600",
        ...
        for (String course : req.options) {
            if (advancedEseCourses.contains(course)) {
                return true;
            }
        }
    ...
    private static boolean isElectricalEngineeringAdvancedEseCourseOptionParsedRule(MajorRequirement req) {
        if (req.sectionName == null) {
            return false;
        }
        ...
        if (note.contains("ese 2900")
                && note.contains("ese 2910")
                && note.contains("introduction to electrical and systems engineering research")) {
            return true;
        }
        ...
        Set<String> advancedEseCourseOptionCourses = new HashSet<>(Arrays.asList(
                "ESE 2900",
                "ESE 2910",
                "ESE 3190",
                "ESE 3360",
        ...
        for (String course : req.options) {
            if (advancedEseCourseOptionCourses.contains(course)) {
                matches++;
            }
        }
    ...
    private static boolean isElectricalEngineeringProfessionalElectiveParsedCourseRow(MajorRequirement req) {
        if (req.sectionName == null || req.options == null) {
            return false;
        }
        ...
        Set<String> professionalElectiveCourses = new HashSet<>(Arrays.asList(
                "ESE 4000",
                "EAS 5450",
                "EAS 5950",
                "MGMT 2370",
        ...
        for (String course : req.options) {
            if (professionalElectiveCourses.contains(course)) {
                return true;
            }
        }
    ...
    private static List<MajorRequirement> removeUnneededBiomedicalScienceParsedRules(
        ...
        for (MajorRequirement req : requirements) {
            ...
            if (note.startsWith("select 2 courses in:")
                    || note.startsWith("select 2 additional courses in:")
                    || note.startsWith("select two additional courses in:")) {
                continue;
            }

            if (sectionName.equals("general requirements")
                    && isUnneededBiomedicalScienceParsedConcentrationRow(req)) {
                continue;
            }
    ...
    private static boolean isUnneededBiomedicalScienceParsedConcentrationRow(MajorRequirement req) {
        if (req.options == null || req.options.isEmpty()) {
            return false;
        }
